...
if __name__ == "__main__":
    ...
    try:
        ...
        for name, model in tqdm(models.items(), desc = "Model Training Progress"):
            ...
            logger.info(f"{name} Model Accuracy: {accuracy}\n")
            ...
            logger.info(f"Plotting confusion matrix for {name}...")
            visualizer.plot_confusion_matrix(y_test, y_pred, class_names=['Class 0', 'Class 1'])

            # Plot classification report
            logger.info(f"Plotting classification report for {name}...")
